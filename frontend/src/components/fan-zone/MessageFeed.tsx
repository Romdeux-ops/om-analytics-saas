...
function sortFeedMessages(msgs: MessageView[]): MessageView[] {
...
export function MessageFeed({ roomId, initialMessages, initialCursor }: MessageFeedProps) {
  ...
  function handleMessagePinned(messageId: number, pinned: boolean) {
    setMessages((prev) =>
      sortFeedMessages(
        prev.map((m) => ({
          ...m,
          is_pinned: m.id === messageId ? pinned : pinned ? false : m.is_pinned,
        })),
      ),
    );
  }
